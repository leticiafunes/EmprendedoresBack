...
emprendedorCtrl.deleteEmprendedor = async (req, res) => {
  await Emprendedor.findByIdAndDelete(req.params.id);
  res.json({ message: ["Borrar Emprendedor"] });
};

emprendedorCtrl.updateEmprendedor = async (req, res) => {
  const {
    nombre,
    activo,
    tags,
  ...
  const respuesta = await Emprendedor.findOneAndUpdate(filter, {
    nombre,
    activo,
    tags,
    resena,
...
emprendedorCtrl.getEmprendedor = async (req, res) => {
